...
Residuals_Plot <- ggplot(fit_2, aes(x = .fitted, y = .resid)) +
  ...
  geom_hline(yintercept = 0, color ="#F0C987") + theme_minimal() + labs(title = "Homoskedasticity Check",subtitle = "Residuals", x="Fitted Values", y="Residuals", caption = "")
...
Normality_plot_Y <- ggplot(df_log, aes(sample = df_log$manufacturing_cost)) + stat_qq() + stat_qq_line()

Normality_plot_X <- ggplot(df_log, aes(sample = df_log$number_of_solar_panels)) + stat_qq() + stat_qq_line()
